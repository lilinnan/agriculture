...
export default {
  ...
  mounted() {
    ...
    if (!this.image) {
      let placeHolder = this.svgToImage(this.placeholderSvg);

      placeHolder.onload = function () {
        let x = self.canvasWidth / 2 - this.width / 2;
        let y = self.canvasHeight / 2 - this.height / 2;
  ...
  methods: {
    ...
    paintImage(context, image, border) {
      if (image.resource) {
        let position = this.calculatePosition(image, border);
        context.save();
    ...
    calculatePosition(image, border) {
      ...
      let heightDiff = (height - dimensions.height) / 2;
